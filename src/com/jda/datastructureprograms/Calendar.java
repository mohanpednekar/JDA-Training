...
class Calendar {
  ...
  private
  StringBuilder generateMonthPage() {
  ...
  void fillUsingQueue() {
    ...
    while (endDate < nDays) {
      ...
      endDate += 7;
      if (endDate > nDays) { endDate = nDays; }
      weeks.enqueue(new Week(DayOfWeek.SUNDAY, startDate, endDate));
    }
  }

  public
  void displayUsingQueue() {
    while (!weeks.isEmpty()) { System.out.println(weeks.dequeue()); }
  }
}
